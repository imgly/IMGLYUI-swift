...
public extension AssetLibrarySource<PhotoRollDestination, PhotoRollPreview, PhotoRollAccessory> {
  ...
  /// - Parameters:
  ///   - mode: The display mode which defines the section title(s).
  ///   - media: The allowed media type(s).
  /// - Returns: The created `AssetLibrarySource`.
  static func photoRoll(_ mode: Mode, media: [PhotoRollMediaType]) -> Self {
    self.init(mode, source: .init(
      id: PhotoRollAssetSource.id,
      config: .init(groups: media.map(\.rawValue)),
    )) {
